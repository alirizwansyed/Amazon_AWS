...
filename=r"C:\users\anumulapellis\downloads\bestprice_result_trade_offers.csv"
...
for i in range(1,(search_results)+1):
    try:
        ...
        margin_perc_lst.append(round((1-(float(sp.replace("₹","").replace(",","").strip(" "))/float(mrp.replace("₹","").replace(",","").strip(" "))))*100,2))
